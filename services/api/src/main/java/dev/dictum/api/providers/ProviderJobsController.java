...
public class ProviderJobsController {
	...
	public ProviderJobsResponse listProviderJobs() {
		List<ProviderStatusResponse> providers = providerAdapters.stream()
			.map(adapter -> new ProviderStatusResponse(
				adapter.key(),
				adapter.mode(),
				adapter.supportedCommands()
			))
			.toList();

		return new ProviderJobsResponse(
			providers,
			List.of(new ProviderJobSummaryResponse(
				"provider-job-preview",
				"noop",
				"idle",
				"Awaiting live provider configuration."
			))
		);
	}
	...
	public record ProviderJobsResponse(
		...
		List<ProviderJobSummaryResponse> jobs
	) {
	}
}
